...
def update_area_coverage(hdf_file, latitude, longitude):
    """Update the geographic extents of the S-104 file.
    
    :param hdf_file: The S-104 HDF file.
...
def update_temporal_coverage(hdf_file, start_time, end_time):
    """Update the temporal extents of the S-104 file.
...
def update_current_speed(hdf_file, min_speed, max_speed):
    """Update the min/max current speed values of the S-104 file.
...
def add_series_group(hdf_file, time_file):
    """Add a new timeseries group to the given S-104 HDF file.
    ...
    numWaterLevelStations = hdf_file.attrs['numberOfStations']
    
    #If this is the first station, then we need to initialize a few things.
    if numWaterLevelStations == 0:
    ...
    #Else this is not a new file, so lets verify a few things.
        ...
        if dataCodingFormat != 1:
            raise Exception('The specified S-104 file does not contain time series data.')
        ...
        if intervalInSeconds != timeRecordInterval:
            raise Exception('The specified S-104 file does not match the input time interval.')

        #Update the XY group with the position information of this time series file.
        xy_group = hdf_file['Group XY']

        x_dataset = xy_group['X']
        x_dataset.resize((1, numWaterLevelStations+1))
        x_dataset[(0, numWaterLevelStations)] = time_file.longitude

        y_dataset = xy_group['Y']
        y_dataset.resize((1, numWaterLevelStations+1))
        y_dataset[(0, numWaterLevelStations)] = time_file.latitude
    ...
    numWaterLevelStations += 1
    hdf_file.attrs.create('numberOfStations', numWaterLevelStations, dtype=numpy.int64)
        
    #Create the new group
    newGroupName = 'Group ' + str(numWaterLevelStations)
    newGroup = hdf_file.create_group(newGroupName)
    
    #Store the title
    newGroupTitle = 'Station No. ' + str(numWaterLevelStations)
    ...
    print("Created tide station group #", str(numWaterLevelStations))
...
def add_series_datasets(group, time_file):
    ...
    :param time_file: The input ASCII file containing the timeseries data.
    :returns: A tuple containing the minimum and maximum waterlevel values added.
    """
    ...
    # For a matrix with n rows and m columns, shape will be (n,m). The length of the shape tuple is therefore the rank, 
...
def create_command_line():
    ...
    parser = argparse.ArgumentParser(description='Add S-104 time series dataset')
...
def main():
    ...
    with h5py.File(results.inOutFile[0], "r+") as hdf_file:
        ...
        time_file = ascii_time_series_wl.AsciiTimeSeriesWL(results.time_series_file)
